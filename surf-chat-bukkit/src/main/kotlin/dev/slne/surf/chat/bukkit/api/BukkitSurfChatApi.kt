...
class BukkitSurfChatApi(): SurfChatApi, Fallback {
    ...
    override fun sendText(player: Player, message: Component, uuid: UUID) {
        ...
        plugin.launch {
            surfChatApi.logMessage(player.uniqueId, ChatMessageType.INTERNAL, finalMessage)
        }
        ...
        historyService.logCaching(uuid, finalMessage)
    }

    override fun sendRawText(player: Player, message: Component, uuid: UUID) {
        plugin.launch {
        ...
        historyService.logCaching(uuid, message)
    }
}