package dev.slne.surf.chat.fallback.repository.ignore
...
class IgnoreRepositoryImpl : IgnoreRepository {
    override suspend fun ignore(uuid: UUID, ignoredUUID: UUID): Boolean = suspendTransaction {
        try {
    ...
    override suspend fun unignore(uuid: UUID, ignoredUUID: UUID): Boolean = suspendTransaction {
        IgnoreListTable.deleteWhere {
            (IgnoreListTable.userUuid eq uuid) and (IgnoreListTable.ignoredUuid eq ignoredUUID)
        } > 0
    }

    override suspend fun findAllByUuid(uuid: UUID): List<IgnoreListEntry> = suspendTransaction {
